...
import { createPreference } from "@/components/MercadoPagoSDK";
...
export default function PaymentQuotasPage(_props: PaymentQuotasProps) {
  ...
  const getTotal = () => {
    ...
    // Sumamos el totalAmount de todas las cuotas seleccionadas
    let total = 0;
    for (const idx of selectedQuotas) {
      // Limpiar el monto (quitar símbolos, puntos, etc) y convertir a número
      const amount = userData.quotas[idx].totalAmount;
  ...
  const getTotalInteres = () => {
    ...
    // Sumamos el interestAmount de todas las cuotas seleccionadas
    let total = 0;
    for (const idx of selectedQuotas) {
      // Limpiar el monto (quitar símbolos, puntos, etc) y convertir a número
      const amount = userData.quotas[idx].interestAmount;
      const cleanAmount = amount.replace(/[^0-9]/g, '');
      if (cleanAmount) {
        total += parseInt(cleanAmount, 10);
      }
